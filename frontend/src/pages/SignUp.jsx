    ...
    <div className='w-full min-h-screen bg-[#0F172A] flex items-center justify-center px-4'>
     <div className='w-full max-w-[500px] card p-6 sm:p-8 flex flex-col gap-8'>
        <div className='w-full rounded-[16px] bg-[#6366F1] shadow-lg flex items-center justify-center py-10 px-4'>
           <h1 className='text-slate-100 font-bold text-3xl text-center'>Welcome to <span className='text-white'>{APP_NAME}</span></h1>
        </div>
        <form className='w-full flex flex-col gap-[20px] items-center' onSubmit={handleSignUp}>

        <input type="text" placeholder='Username' className='w-full h-[52px] outline-none px-[20px] py-[10px] input-modern text-[17px]' onChange={(e)=>setUserName(e.target.value)} value={userName}/>
        ...
        <button className='px-[20px] py-[12px] btn-primary text-[18px] w-full mt-[6px] font-semibold transition-colors' disabled={loading}>{loading?"Loading...":"Sign Up"}</button>
        <p className='cursor-pointer text-slate-300 text-center' onClick={()=>navigate("/login")}>Already have an account? <span className='text-[#22C55E] font-semibold'>Login</span></p>
     </form>
     </div>
